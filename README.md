## Ambiente 
 ...
 Aplicação foi testado com JRE9 no Apache Tomcat 9
...
O testes do projeto usa MySQL no endereco IP (localhost ou 127.0.0.1) e deve existir já o banco `casadocodigo_test` (usuario `casadocodigo_test`sem senha). O JPA foi configurado para dropar e gerar as tables automaticamente ao iniciar (na classe `JPAConfiguration`). Caso queira gerar as tabelas manualmente, abaixo desse README tem os comandos SQL.

## Compilação com e sem testes

 Para compilar immporte o projeto no Eclipse (*Import as Maven Projeto*) ou compile na linha de comando usando Maven:

 	Com testes: Necessario habilitar banco mysql para testes
...
Execute a "URL Mágica" para cadastrar produtos e um usuario padrão (Login: admin@casadocodigo.com.br, Senha: 123456)
 ...
 A "URL Mágica" se econtra na configuração "SecurityConfiguration".
